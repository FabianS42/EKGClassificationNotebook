...
import numpy as np
import TimeSeries.TransformerVisualization as tv
...
class EncoderTransformer(tf.keras.Model):
  ...
  def __init__(self, model_dimension:int, num_layers:int, num_heads:int, encoder_feed_forward:int,
               model_output_size:int, encoder_dropout_rate : float=0.2, output_feed_forward_layer : list = [0] ,  dropout_rate_after_enocder : float = 0.0, cnn_layer_after_encoder:int = 0, classification:bool=False,  segment_sizes:int=1, segment_stride:int=None, cls_token:bool=False, add_positional_encoding:bool=False):
    super().__init__()
    ...
    if classification:      
      ...
      self.final_layer = tf.keras.layers.Dense(model_output_size, activation='softmax', name = "classification_layer")
    else:        
      if self.output_feed_forward_layer[0] > 0:
        self.output_dense_layer = [tf.keras.layers.Dense(fc, activation='relu', name = "feed_forward_layer") for fc in output_feed_forward_layer ]         
      self.final_layer = tf.keras.layers.Dense(model_output_size, name = "output_layer")
  ...
  def call(self, inputs, return_scores_and_feature_vectore=False):
    ...
    #Seperate cls token for klassification layer or take the all tokens for klassification
    if self.cls_token == True:
  ...
  def predict_return_attention_scores_and_feature_vectore(self, x, batch_size = 16):
    ...
    for i in np.arange(0,len(x)-batch_size,batch_size):
  ...
  def visualize_attention_score_within_timeseries(self, x_test, y_test, number_of_samples_to_plot_from_each_class = 5, x_test_variable_to_plot = 0 ):
    ...
    """
    pred, attention_score, _ = self.predict_return_attention_scores_and_feature_vectore(x_test, batch_size=128)
  ...
  def visualize_encoder_output_feature_vector_within_timeseries_with_PCA_to_RRG(self, x_test, y_test, number_of_samples_to_plot_from_each_class = 5, x_test_variable_to_plot= 0):
    ...
    """
    pred, _, encoder_output_feature_vector = self.predict_return_attention_scores_and_feature_vectore(x_test, batch_size=128)
    if len(x_test.shape) > 2:
      ts = np.squeeze(x_test[:,:,x_test_variable_to_plot])
